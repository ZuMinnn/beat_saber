...
const Debris: React.FC<{ data: NoteData, timeSinceHit: number, color: string }> = ({ data, timeSinceHit, color }) => {
    ...
    return (
        <group ref={groupRef}>
            ...
            <Shard offsetDir={[0, 0, 0]} moveDir={[-0.5, 0.8, -0.5]} />
            <Shard offsetDir={[0, 0, 0]} moveDir={[0.8, -0.5, -0.5]} />
            <Shard offsetDir={[0, 0, 0]} moveDir={[-0.8, -0.5, -0.5]} />
...
const Note: React.FC<NoteProps> = ({ data, zPos, currentTime, color }) => {
  ...
  return (
    <group position={position}>
      {/* Matte Plastic Box */}
      <RoundedBox args={[NOTE_SIZE, NOTE_SIZE, NOTE_SIZE]} radius={0.1} smoothness={4}>
          <meshToonMaterial color={color} />
      </RoundedBox>

      {/* Center Dot for visual focus */}
      <mesh position={[0, 0, NOTE_SIZE / 2 + 0.01]}>
         <circleGeometry args={[NOTE_SIZE * 0.3, 16]} />
