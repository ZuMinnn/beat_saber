...
const Saber: React.FC<SaberProps> = ({ type, positionRef, velocityRef, config }) => {
  ...
  return (
    <group ref={meshRef}>
      ...
      {/* --- TRAIL SYSTEM --- */}
      {/* We anchor the trail to the center of the blade to create a "ribbon" following the motion */}
      
      {/* 1. Outer Glow Trail - Wide and transparent */}
      <group position={[0, 0.05 + saberLength / 2, 0]}>
        <Trail
            width={1.2 * thickness} // Wide trail
            length={6} // Length in frames
            color={color}
            attenuation={(t) => t * t} // Quadratic fade out
        >
            <mesh visible={false}>
      ...
      {/* 2. Inner Core Trail - Thin and bright white */}
      <group position={[0, 0.05 + saberLength / 2, 0]}>
        <Trail
            width={0.6 * thickness} 
            length={4} 
